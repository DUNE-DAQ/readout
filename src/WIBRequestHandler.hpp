...
protected:
  ...
  RequestResult tpc_data_request(dfmessages::DataRequest dr, unsigned delay_us)
  {
    ...
    dataformats::WIBHeader front_wh =
      *(reinterpret_cast<const dataformats::WIBHeader*>(m_latency_buffer->getPtr(0)));                        // NOLINT
    ...
    uint64_t newest_ts =                                                                                      // NOLINT
      last_ts + (occupancy_guess - m_safe_num_elements_margin) * m_tick_dist * m_frames_per_element;          // NOLINT
    ...
    if (rres.result_code != ResultCode::kFound) {
      ...
      for (uint32_t idxoffset = 0; idxoffset < num_elements_in_window; ++idxoffset) { // NOLINT
        ...
        if (element != nullptr) {
          frag_pieces.emplace_back(std::make_pair<void*, size_t>(
            static_cast<void*>(m_latency_buffer->getPtr(num_element_offset + idxoffset)), std::size_t(m_element_size)));
          elements_handled++;
