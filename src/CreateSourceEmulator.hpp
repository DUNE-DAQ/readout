...
createSourceEmulator(const appfwk::app::QueueInfo qi, std::atomic<bool>& run_marker)
{
  ...
  if (inst.find("wib2") != std::string::npos) {
    ...
    auto source_emu_model = std::make_unique<SourceEmulatorModel<types::WIB2_SUPERCHUNK_STRUCT>>(qi.name, run_marker, 32, 0.0, 166.0);
  ...
  if (inst.find("wib") != std::string::npos) {
    ...
    auto source_emu_model = std::make_unique<SourceEmulatorModel<types::WIB_SUPERCHUNK_STRUCT>>(qi.name, run_marker, 25, 0.0, 166.0);
    return std::move(source_emu_model);
  }
  ...
  if (inst.find("pds") != std::string::npos) {
    ...
    auto source_emu_model = std::make_unique<SourceEmulatorModel<types::PDS_SUPERCHUNK_STRUCT>>(qi.name, run_marker, 16, 0.9, 200.0);
    return std::move(source_emu_model);
  }
