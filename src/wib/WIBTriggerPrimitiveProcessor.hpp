...
class WIBTriggerPrimitiveProcessor : public TaskRawDataProcessorModel<types::RAW_WIB_TP_TYPE>
{

public:
  using inherited = TaskRawDataProcessorModel<types::RAW_WIB_TP_TYPE>;
  using frame_ptr = types::RAW_WIB_TP_TYPE*;
  ...
  explicit WIBTriggerPrimitiveProcessor(std::unique_ptr<FrameErrorRegistry>& error_registry)
    : TaskRawDataProcessorModel<types::RAW_WIB_TP_TYPE>(error_registry)
  {
    TaskRawDataProcessorModel<types::RAW_WIB_TP_TYPE>::add_preprocess_task(
      std::bind(&WIBTriggerPrimitiveProcessor::tp_stitch, this, std::placeholders::_1));
  }
  ...
  void init(const nlohmann::json& args) override
  {
    try {
      ...
      if (queue_index.find("tp") != queue_index.end()) {
        m_tp_source.reset(new appfwk::DAQSource<types::RAW_WIB_TP_TYPE>(queue_index["tp"].inst));
      }
...
private:
  using source_t = appfwk::DAQSource<types::RAW_WIB_TP_TYPE>;
