...
main(int /*argc*/, char** /*argv[]*/)
{
  ...
  auto producer = std::thread([&]() {
    ...
    while (marker) {
      ...
      auto plptr = const_cast<dunedaq::detdataformats::WIBHeader*>(
        reinterpret_cast<const dunedaq::detdataformats::WIBHeader*>(&pl)); // NOLINT
  ...
  auto cleaner = std::thread([&]() {
    ...
    while (marker) {
      ...
      if (tail && head) {
        auto tailptr = reinterpret_cast<const dunedaq::detdataformats::WIBHeader*>(tail); // NOLINT
        auto headptr = reinterpret_cast<const dunedaq::detdataformats::WIBHeader*>(head); // NOLINT
        ...
        if (headts - tailts > max_time_diff) { // ts differnce exceeds maximum
          ...
          while (timediff >= max_time_diff) {
            ...
            tail = cleanacc.last();
            tailptr = reinterpret_cast<const dunedaq::detdataformats::WIBHeader*>(tail); // NOLINT
    ...
    while (marker) {
      { // block enforce for Accessor
        ...
        if (tail && head) {
          ...
          auto trigptr = const_cast<dunedaq::detdataformats::WIBHeader*>(
            reinterpret_cast<const dunedaq::detdataformats::WIBHeader*>(&trigger_pl)); // NOLINT
          ...
          // if (close) {
          auto foundptr = reinterpret_cast<const dunedaq::detdataformats::WIBHeader*>(&(*close)); // NOLINT
