...
namespace dunedaq {
  namespace readout {
    ...
    {
    public:
      explicit TPHandler(appfwk::DAQSink<types::SW_WIB_TRIGGERPRIMITIVE_STRUCT>& tp_sink, appfwk::DAQSink<trigger::TPSet>& tpset_sink, uint64_t tp_timeout, uint64_t tpset_window_size, dataformats::GeoID geoId) : m_tp_sink(tp_sink), m_tpset_sink(tpset_sink), m_tp_timeout(tp_timeout), m_tpset_window_size(tpset_window_size), m_geoid(geoId)
      {}
