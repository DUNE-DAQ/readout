...
struct PACMAN_MESSAGE_STRUCT
{
  ...
  bool operator<(const PACMAN_MESSAGE_STRUCT& other) const
  {
    ...
    return (thisptr->get_msg_header((void*)&data)->unix_ts) < (otherptr->get_msg_header((void*)&other.data)->unix_ts)
             ? true
             : false; // NOLINT
  }
  ...
  uint64_t get_timestamp() const // NOLINT(build/unsigned)
  {
    return (
      (uint64_t)(
        reinterpret_cast<const dunedaq::dataformats::PACMANFrame*>(&data)->get_msg_header((void*)&data)->unix_ts) *
      1000000000); // NOLINT
  }
  ...
  {
    return reinterpret_cast<const dunedaq::dataformats::PACMANFrame*>(&data)
